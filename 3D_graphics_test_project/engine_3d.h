...
namespace cmn {
	...
	inline void Engine3D::TexturedDepthTriangle(int x1, int y1, float u1, float v1, float w1, int x2, int y2, float u2, float v2, float w2, int x3, int y3, float u3, float v3, float w3, olc::Sprite* spr, int id)
	{
		//sort by y
		if (y2 < y1) {
			std::swap(x1, x2);
